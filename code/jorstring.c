...
string_ref copy_c_string_with_length(memory_arena* arena, const char* str, u32 str_size)
{
    ...
    /* str_size has come in corrupt at least once (identical crash at 5mb
       and 24mb arenas points at a runaway length, not a full arena) -
       nothing this game copies here is anywhere near 4mb, so treat
       anything past that as garbage instead of looping on it. */
    if (str == NULL || str_size > megabytes_to_bytes(4))
    {
        return result;
    }
