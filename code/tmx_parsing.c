...
xml_node_search_result* find_all_nodes_with_tag(memory_arena* arena, xml_node* root_node, const char* tag)
{
    ...
    {
        /* first pass: count matches so we can push one real contiguous
           array. the old code called push_struct(arena, xml_node*) once
           per match and assumed the bump allocator would lay consecutive
           pointers back-to-back - true on desktop x86_64 where
           sizeof(xml_node*)==8 matches the arena's 8-byte alignment (no
           gap), but false on vita's 32-bit ARM target where
           sizeof(xml_node*)==4: push_size_impl re-aligns to 8 bytes
           before every push, so a 4-byte gap opens up before the 2nd,
           4th, etc. pointer and found_nodes[index] reads garbage/wrong
           nodes for any match past the first. */
        u32 match_count = 0;
        xml_node* counting_child = root_node->first_child;
        while (counting_child)
        {
            if (compare_to_c_string(counting_child->tag, tag))
            {
                match_count++;
            }
            counting_child = counting_child->next;
        }

        if (match_count > 0)
        {
            result->found_nodes = push_array(arena, match_count, xml_node*);
            ...
            while (child)
            {
                if (compare_to_c_string(child->tag, tag))
                {
                    result->found_nodes[result->found_nodes_count] = child;
