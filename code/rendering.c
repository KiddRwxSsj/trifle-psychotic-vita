...
void* push_render_element(render_list* render, u32 size, render_list_entry_type type)
{
    ...
    // header_size and size are forced to a 4-byte boundary before they touch
    // either the pointer math or the running byte offset. Without this, a
    // single non-word-sized header/entry poisons push_buffer_size for the
    // rest of the frame - every later push_render_element() call inherits
    // the same byte-level drift, and it eventually lands an STM/LDM (used to
    // copy rect/v4 fields into the entry) on an unaligned, sometimes odd,
    // address - a hard Data Abort.
    u32 header_size = (sizeof(render_list_entry_header) + 3) & ~3u;
    u32 aligned_size = (size + 3) & ~3u;

    if ((render->push_buffer_size + aligned_size + header_size) < render->max_push_buffer_size)
    {
        ...
        // move past the header by its ALIGNED size, not raw sizeof(header)
        result = (render_list_entry_header*)((byte*)result + header_size);
        render->push_buffer_size += (aligned_size + header_size);
    }
    else
    {
        invalid_code_path;
