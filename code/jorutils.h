...
{
    /* base comes straight from the platform's malloc/SDL_malloc call. If that
       allocation failed and returned NULL, the write below (arena->size = size)
       is a write to address 0 - an instant, unexplained crash on boot, before
       push_size_impl's overflow check ever gets a chance to run. Fail loud
       instead: same *(int*)0=0 trap the rest of this codebase uses under
       TRIFLE_DEBUG, but the caller is expected to check its own allocation
       first (see vita_platform.c) so this is a backstop, not the primary fix. */
    if (base == NULL)
    {
        assert(!"initialize_memory_arena: base is NULL (allocation failed)");
        return NULL;
    }
...
{
    memory_index aligned_size_used = (arena->size_used + 7) & ~(memory_index)7;

    /* bounds check must run unconditionally, in every build, and BEFORE
       any assert - under TRIFLE_DEBUG=1 assert() is a live *(int*)0=0
       crash, so if it ran first this check never executed. */
    if (arena->base == NULL || (aligned_size_used + size_to_push) > arena->size)
    {
        assert(!"push_size_impl: arena overflow");
        return NULL;
    }

    void* result = arena->base + aligned_size_used;
    arena->size_used = aligned_size_used + size_to_push;
    return result;
}
